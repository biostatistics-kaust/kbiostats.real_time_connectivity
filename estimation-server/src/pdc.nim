...
proc matrix_spectrum_122*[T: SomeFloat](X: Tensor[T], f: T = 0.1): Tensor[Complex[T]] =
  ...
  var
    weights_sin = zeros[T](p, m - i0)
    weights_cos = zeros[T](p, m - i0)
  result = zeros[T](p, p)
  for k in 0..<int(m/p):
    weights_cos[_.._, (k * p)..<((k + 1) * p)] = cos(T(- PI) * f * T(k + 1))
    weights_sin[_.._, (k * p)..<((k + 1) * p)] = sin(T(- PI) * f * T(k + 1))
  weights_cos = weights_cos *. X[_.._, i0..<m]
  weights_sin = weights_sin *. X[_.._, i0..<m]
  #for k in 0..<p:
  #  result[_.._, k] =  weights[_.._, k..<m|p].sum(1)
  
proc matrix_spectrum_orig*[T: SomeFloat](X: Tensor[Complex[T]], f: T = 0.1): Tensor[Complex[T]] =
  # f can be from 0 to 1 (fs)
  # TODO: Optimize
  let
    p = X.shape[0]
...
proc matrix_spectrum*[T: SomeFloat](X: Tensor[Complex[T]], f: T = 0.1): Tensor[Complex[T]] =
  ...
  let
    ...
    m = X.shape[1]
    i0 = m mod p #if include_intercept: 1 else: 0
    idx = toSeq(1..int(m/p)).toTensor.astype(Complex[T])
    indices = toSeq(1..p).toTensor.astype(T)
  var
    fourier_weights = exp(im(T(- PI) * f) * idx)
    #fourier_weights = cos(T(- PI) * f * indices).astype(Complex[T]) + (im(T(1)) * sin(T(- PI) * f * indices).astype(Complex[T]))
    weights = zeros[Complex[T]](p, m - i0)
  result = zeros[Complex[T]](p, p)
  for k in 0..<int(m/p):
    weights[_.._, (k * p)..<((k + 1) * p)] = im(T(- PI) * f * T(k + 1))
    #echo "::", (k * p), "..<", ((k + 1) * p), "  p:", p, "  k:", k
  #weights = weights.exp *. X[_.._, i0..<m]
  #weights = weights.exp *. X[_.._, i0..<m]
  #########echo "$$$"
  #########echo fourier_weights
  for k in 0..<p:
    #########echo X[k, i0..<m|p].sum(0).reshape(int(m/p))
    for k1 in 0..<p:
      result[k, k1] = sum(fourier_weights *. X[k, (i0+k1)..<m|p].sum(0).reshape(int(m/p))) #weights[_.._, k..<m|p].sum(1)
    #result[k, _.._] = fourier_weights *. ((X[k, i0..<m|p].sum(0)).reshape(int(m/p))) #weights[_.._, k..<m|p].sum(1)
    #result[_.._, k] = complex(T(k)) #weights[_.._, k..<m|p].sum(1)
  #########echo result
  
proc matrix_spectrum*[T: SomeFloat](X: Tensor[T], f: T = 0.1): Tensor[Complex[T]] =
  matrix_spectrum(X.astype(Complex[T]), f)

proc matrix_spectrum_range_orig*[T: SomeFloat](X: Tensor[T], f0: T = 0, f1: T = 0.1, n_points: int = 100): Tensor[T] =
  let
